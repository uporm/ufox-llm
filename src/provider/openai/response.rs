//! `OpenAI` 响应反序列化模块。
//!
//! 该模块负责将 `OpenAI Chat Completions` 的非流式响应体解析为 SDK 公共响应模型。
//!
//! 设计上采用“先解析私有结构，再映射公共类型”的方式：
//! 1. 先以 `OpenAI` 私有结构体承接响应，避免公共类型直接耦合第三方字段命名；
//! 2. 再把首个 `choice` 转换为 [`ChatResponse`]，统一提取文本内容、工具调用、
//!    结束原因与用量信息；
//! 3. 当响应体包含业务错误对象时，优先转换为 [`LlmError::ApiError`]，让上层能够
//!    获得稳定的错误语义，而不是仅看到底层反序列化失败。
//!
