...
async fn main() -> Result<()> {
    ...
    let agent = Agent::builder()
        .llm(llm)
        .system("你是一个多模态助手，可以理解图片和文档内容。请用中文回答。")
        .memory(store)
        .enable_perceive(true)
        .build()
        .map_err(|e| anyhow::anyhow!("{e}"))?;

    let mut session = agent.session("alice", "mm-session-001").await?;
